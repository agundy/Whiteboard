...
from django.shortcuts import (
    render_to_response, get_object_or_404, get_list_or_404, redirect
)
    ...
    if request.method == 'POST':
        ...
        if student_form.is_valid():  # Verify form is complete, correct data
            ...
            if len(User.objects.filter(
                    email=student_form.cleaned_data['email'])) != 0:
...
def send_edu_email_confirmation(user):
    ...
    content = """<a href="{0}">Please confirm your "\
        ".edu email address!</a><br>""".format(
        """{0}/confirm_email/{1}/{2}/""".format(
            SITE_ADDR, str(p.confirmation_code), user.username)
    )
    send_mail(title, content, 'no-reply@%s.com' %
              SITE_ADDR, [user.email], fail_silently=False)
...
def profile(request, pk):
    ...
    if pk is None:
        ...
        return render_to_response('Student/not_found',
                                  {'errors': errors},
                                  RequestContext(request))
...
def show_student_groups(request):
    return render_to_response('Student/groups.html',
                              RequestContext(request))
...
def join_school(request):
    ...
    if request.POST:
            ...
            if request.POST['school'] != '':
                ...
                if form.is_valid():
                    ...
                    if email.split("@")[1] == school.email_domain:
                        ...
                        form = school_form = JoinSchoolForm(
                            initial={'email': email})

                        return render_to_response(
                            'Student/join_school.html',
                            {'form': form,
                    ...
                    return render_to_response('Student/join_school.html',
                                              {'form': school_form,
                                               'message': message,
                                               'email': email,
                                               'errors': errors},
        ...
        if ".edu" in request.user.email.lower():
            ...
            email = None
            message = "Select your school and enter "\
                "a matching .edu email address."

        school_form = JoinSchoolForm(initial={'email': email, 'school': None})

        return render_to_response('Student/join_school.html',
                                  {'form': school_form, 'message': message,
                                   'email': email, 'errors': errors},
                                  RequestContext(request))
...
def add_student_item(request, courseitem_pk):
    ...
    studentitem = StudentItem.objects.create(
        courseitem=courseitem, state=0, score=None)
    student.assignments.add(studentitem)

    return redirect("/course/section/show/" +
                    str(courseitem.courseInstance.pk))
...
def remove_student_item(request, studentitem_pk):
    ...
    studentitem.delete()
    return redirect("/course/section/show/" +
                    str(studentitem.courseitem.courseInstance.pk))
...
def edit_student_item(request, studentitem_pk):
    ...
    if request.POST:
        ...
        student_item_form = StudentItemForm(
            request.POST, student=student, section=section)
        if student_item_form.is_valid():
            ...
            student_item.description = student_item_form.cleaned_data[
                'description']
            student_item.assignment_type = student_item_form.cleaned_data[
                'assignment_type']
            ...
            # Could be optimized for speed if the score or assignment type
            # wasn't changed.
            update_grades(
                student.pk, student_item.courseitem.courseInstance.pk)

            return redirect("/course/section/show/" +
                            str(student_item.courseitem.courseInstance.pk))
        else:
            studentitem = StudentItem.objects.get(
                student=student, pk=studentitem_pk)
            return render_to_response("Student/edit_studentitem.html",
...
def show_grades(request):
    ...
    for section in sections:
        try:
            assignments = get_list_or_404(
                student_assignments, courseitem__courseInstance=section.course)
        except:
            assignments = None
        ...
        section_grades.append(
            (section, assignments, assignment_types, overall_grade))
    return render_to_response("Student/grades.html",
                              {'student': student,
                               'sections': sections,
    ...
    if request.POST:
        ...
        if assignment_type_form.is_valid():
            assignment_type, created = AssignmentType.objects.get_or_create(
                ...
                weight=assignment_type_form.cleaned_data[
                    'weight'],
                name=assignment_type_form.cleaned_data['name'],
                student=student)
            assignment_type.weight = assignment_type_form.cleaned_data[
                'weight']
        ...
        return render_to_response("Student/assignment_type.html",
                                  {'assignment_type_form':
                                   assignment_type_form, 'section': section},
                                  RequestContext(request))
