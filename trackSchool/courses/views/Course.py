...
def browse_courses(request):
    ...
    return render_to_response('Course/browse_courses.html',
                              {'enrolled': enrolled,
                               'courses': courses,
                               'school': student.school},
                              RequestContext(request))
...
def show_section(request, pk):
    ...
    courseItems = CourseItem.objects.filter(
        courseInstance=section.id).exclude(id__in=excluded_items)
    ...
    for student_item in list(student_items):
        student_item_form_pair.append((student_item, StudentItemForm(
            student=student,
...
def join_section(request, pk):
    ...
    if pk is None:
        print "Error no course"
        errors = ['No course selected']
        return render_to_response('Course/section_not_found.html',
                                  {'errors': errors}, RequestContext(request))
    student = get_object_or_404(Student, user=request.user)
    section = get_object_or_404(Section, id=pk)
    ...
    student_section = StudentSection.objects.get_or_create(
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            section.slug = slugify(
                section.course.dept + str(section.course.courseID) + str(section.id_no))
            section.save()
            ...
            return render_to_response('Course/add_section.html',
                                      {'form': form, 'course':
                                       course, 'errors': errors},
                                      RequestContext(request))
    else:
        form = CreateSectionForm(
            initial={'course': course, 'year': datetime.now().strftime('%Y')})
        return render_to_response('Course/add_section.html',
                                  {'course': course,
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            due_date = datetime.combine(
                form.cleaned_data['due_date'], form.cleaned_data['due_time'])

            assignment, created = CourseItem.objects.get_or_create(
                name=form.cleaned_data['name'],
                due_date=due_date,
                point_value=form.cleaned_data[
                    'point_value'],
