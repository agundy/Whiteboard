...
from courses.models import (Student, Section, CourseItem,
                            AssignmentType, StudentSection)
import datetime
...
def update_grades(student_pk, section_pk):
    ...
    student_section = StudentSection.objects.get(
    ...
    for assignment_type in assignment_types:
        ...
        assignments = list(student.assignments.filter(
            assignment_type=assignment_type,
            courseitem__courseInstance=section))
        total_possible = 0
        total_score = 0
        ...
        if total_possible > 0:
            assignment_type.aggregate_grade = total_score / \
                total_possible * 100
...
def update_priority(student_pk):
    ...
    for section in student_sections:
        assignment_types = list(
            AssignmentType.objects.filter(sectionInstance=section))
        ...
        for assignment_type in assignment_types:
            ...
            for assignment in assignments:
                due_date = assignment.courseitem.due_date
                time_now = datetime.now()
                time_left = (due_date - time_now).total_seconds() / 3600
